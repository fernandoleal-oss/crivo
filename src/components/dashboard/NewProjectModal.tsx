        ...
        <DialogContent>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 pt-2">
            <div>
              ...
              <p className="text-xs text-slate-500 mb-1">Identifique a campanha ou demanda. Use algo descritivo para facilitar a busca depois.</p>
              <Input id="name" value={name} onChange={e => setName(e.target.value)} placeholder="Ex: Campanha Janeiro 2026" required />
            </div>
            <div>
              <Label htmlFor="client">Cliente</Label>
              <p className="text-xs text-slate-500 mb-1">Nome da empresa ou pessoa que vai aprovar as peças.</p>
              <Input id="client" value={clientName} onChange={e => setClientName(e.target.value)} placeholder="Ex: EMS Pharma" required />
            </div>
            <div>
              <Label>Setor responsável</Label>
              <p className="text-xs text-slate-500 mb-1">Selecione a área da agência que cuida desse projeto.</p>
